...
function Install-ConsoleFreeLauncher([string]$InstallRoot) {
    $launcherPath = Join-Path $InstallRoot "WLX_Launcher.vbs"
    $launcherSource = @"
Option Explicit
...
bootstrapPath = filesystem.BuildPath(scriptDirectory, "WLX_Bootstrap.ps1")
powershellPath = shell.ExpandEnvironmentStrings("%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe")

If Not filesystem.FileExists(bootstrapPath) Then
    MsgBox "The WLX launcher is missing WLX_Bootstrap.ps1. Run INSTALL_OR_UPDATE.bat again.", 16, "Willex's Whimsical Arts"
    WScript.Quit 1
End If
...
' Window style 0 starts PowerShell hidden before Windows can paint a console.
shell.Run command, 0, False
"@
    $utf8NoBom = New-Object System.Text.UTF8Encoding($false)
    [IO.File]::WriteAllText($launcherPath, $launcherSource, $utf8NoBom)
    return $launcherPath
}
...
function New-LaunchShortcut([string]$InstallRoot, [string]$IconPath, [string]$ExePath) {
    ...
    $launcherPath = Install-ConsoleFreeLauncher $InstallRoot
    $wscriptPath = Join-Path $env:SystemRoot "System32\wscript.exe"
    if (-not (Test-Path -LiteralPath $wscriptPath -PathType Leaf)) {
        $wscriptPath = [string](Get-Command "wscript.exe" -CommandType Application -ErrorAction Stop).Source
    }
    $shell = New-Object -ComObject WScript.Shell
