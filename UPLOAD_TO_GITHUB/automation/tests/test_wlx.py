...
class CorePublisherTests(unittest.TestCase):
    ...
    def test_cockatrice_installer_contains_verified_bootstrap(self) -> None:
        ...
        with zipfile.ZipFile(archive_path) as archive:
            ...
            self.assertIn("Install-ConsoleFreeLauncher", updater)
            self.assertIn('Join-Path $InstallRoot "WLX_Launcher.vbs"', updater)
            self.assertIn('$shortcut.TargetPath = $wscriptPath', updater)
            self.assertIn('$shortcut.Arguments = "//nologo `\"$launcherPath`\""', updater)
            self.assertIn("shell.Run command, 0, False", updater)
            self.assertIn("-ExecutionPolicy Bypass -STA -WindowStyle Hidden", updater)
            self.assertNotIn('$shortcut.TargetPath = $powershellPath', updater)
