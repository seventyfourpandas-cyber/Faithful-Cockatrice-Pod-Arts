...
$installRoot = Join-Path $env:LOCALAPPDATA "__INSTALL_FOLDER__"
New-Item -ItemType Directory -Path $installRoot -Force | Out-Null
$legacyInstallRoot = ""
if ("__LEGACY_INSTALL_FOLDER__") {
    $candidateLegacyRoot = Join-Path $env:LOCALAPPDATA "__LEGACY_INSTALL_FOLDER__"
...
if ([int]$manifest.schema_version -ne 1 -or [string]$manifest.package_id -ne "__PACKAGE_ID__") {
...
if ($installedHash -ne $expectedHash) {
    ...
    try {
        ...
        if ([string]$downloadedConfig.package_id -ne "__PACKAGE_ID__") {
