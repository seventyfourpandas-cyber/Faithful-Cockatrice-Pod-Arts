...
function New-LaunchShortcut([string]$InstallRoot, [string]$ExePath) {
    ...
    $shortcutPath = Join-Path $desktop "Willex's Whimsical Arts.lnk"
...
$settings = $null
if (Test-Path -LiteralPath $settingsPath -PathType Leaf) { $settings = Read-JsonFile $settingsPath }
...
if ([string]$manifest.package_id -ne "alex-cockatrice-alt-art") { throw "The hosted manifest belongs to a different package." }
